...
void Server::noticeExec(Client &client, std::vector<std::string> &args) {
	...
	for(; it != ite; it++) {
		if((*it).find_first_of("&#+!") == 0) {  //if channel name
			...
			channelDest->sendMsgToChan(prefixCompose(client) + " NOTICE " + (*it) + " :" + args[2] + "\n", &client, false);
		}
		else {  //if user nick
			Client *clientDest = findClient(*it);
			if (clientDest)
				sendMessage((":" + client.getNick() + "!" + client.getUsername() + "@" + client.getHost() + " NOTICE " + (*it) + " :" + args[2] + "\n"), clientDest->getSockFd());
			else
				throw static_cast<std::string>(ERR_NOSUCHNICK(*it));
		}
	}
